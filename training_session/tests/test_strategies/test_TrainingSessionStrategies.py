...
class CreateModelStageStrategyTestCase(TestCase):
    def setUp(self):
        self.executor = StrategyExecutor()
        self.session_entity = TrainingSessionEntity()
        self.strategy_request = StrategyRequestEntity()
        self.strategy_request.strategy_name = "CreateModelStageStrategy"
        self.strategy = CreateModelStageStrategy(self.executor, self.strategy_request)

    def test_apply(self):
        self.strategy.apply(self.session_entity)
        
        # Get model stage using get_children_by_type
        model_stages = self.session_entity.get_children_by_type(EntityEnum.MODEL_STAGE)
        
        self.assertEqual(len(model_stages), 1)