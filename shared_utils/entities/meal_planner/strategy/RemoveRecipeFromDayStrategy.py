...
class RemoveRecipeFromDayStrategy(Strategy):
    """
    Strategy for removing a recipe ID from a specific day of the week in a MealPlanEntity.
    """

    entity_type = EntityEnum.MEAL_PLAN
    strategy_description = 'Removes a recipe ID from a specific day of the week in a meal plan'
    ...
    def verify_executable(self, entity: Entity, strategy_request: StrategyRequestEntity):
        ...
        if 'recipe_id' not in config:
            raise ValueError("param_config must include 'recipe_id' to remove from the day")
    ...
    def apply(self, entity: Entity) -> StrategyRequestEntity:
        """
        Remove a recipe ID from the specified day of the week in the meal plan.
        """
        ...
        # Check if the recipe exists on that day before attempting removal
        current_recipes = entity.get_recipes_for_day(day)
        recipe_existed = recipe_id in current_recipes
        
        # Use the entity's method to remove the recipe
        entity.remove_recipe_from_day(day, recipe_id)
        ...
        self.strategy_request.ret_val['recipe_existed'] = recipe_existed
    ...
    def get_request_config():
        """
        Default config for RemoveRecipeFromDayStrategy.
        """
        return {
            "strategy_name": "RemoveRecipeFromDayStrategy",
            "strategy_path": None,
            "param_config": {
                "day": "monday",
                "recipe_id": ""
    ...
    def request_constructor(cls, target_entity_id: str, day: str, recipe_id: str, add_to_history: bool = False):
        """
        Convenience method to construct a strategy request for removing a recipe from a day.
        """
