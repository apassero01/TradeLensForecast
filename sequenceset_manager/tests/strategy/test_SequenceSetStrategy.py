...
from data_bundle_manager.strategy.DataBundleStrategy import LoadDataBundleDataStrategy, SplitBundleDateStrategy, \
    CreateFeatureSetsStrategy, ScaleByFeatureSetsStrategy
...
from sequenceset_manager.strategy.SequenceSetStrategy import PopulateDataBundleStrategy, CreateDataBundleStrategy, \
...
class PopulateDataBundleStrategyTestCase(TestCase):
    def setUp(self):
        ...
        self.strategy_executor.register_strategy(
            LoadDataBundleDataStrategy.__name__, LoadDataBundleDataStrategy
        )
        ...
        self.sequence_set_entity_1.set_entity_map({EntityEnum.DATA_BUNDLE.value: DataBundleEntity()})
        self.sequence_set_entity_2.set_entity_map({EntityEnum.DATA_BUNDLE.value: DataBundleEntity()})
    ...
    def test_apply_populates_existing_data_bundles(self):
        ...
        for sequence_set in self.sequence_sets:
            ...
            self.assertIn("X", data_bundle.dataset)
            self.assertIn("y", data_bundle.dataset)
            self.assertIn("row_ids", data_bundle.dataset)
            self.assertIn("X_feature_dict", data_bundle.dataset)
            self.assertIn("y_feature_dict", data_bundle.dataset)
    ...
    def test_verify_executable_raises_with_no_databundle(self):
        self.sequence_set_entity_2.set_entity_map({EntityEnum.DATA_BUNDLE.value: None})
        with self.assertRaises(ValueError):
            self.strategy.verify_executable(self.sequence_sets, self.strategy_request)

        self.sequence_set_entity_2.entity_map = {}
        with self.assertRaises(ValueError):
            self.strategy.verify_executable(self.sequence_sets, self.strategy_request)
