  ...
  export const flowNodesSelector = selector({
    ...
    get: ({ get }) => {
      ...
      entityIds.forEach((id) => {
        ...
        if (!newNode) {
          nodeCache.delete(id); // Remove from cache if entity is gone
          return;
        }
  
        const cachedNode = nodeCache.get(id);
        // Compare nodes to decide if we need a new object
        if (
          !cachedNode ||
          JSON.stringify(cachedNode) !== JSON.stringify(newNode) // Coarse but effective
        ) {
          console.log('Updating or adding node:', id);
          nodeCache.set(id, { ...newNode }); // Create a new object
        }
        // Keep the cached node if unchanged
